...
const Otp = () => {
    ...
    const verifyOtp = async () => {
        try {
            ...
            const response = await axios.post('http://localhost:3001/api/verify-otp', { email: userEmail, enteredOTP });
            ...
            <div className="otp_container">
                <div className="otp_glass">
                    <div className="otp_title">
                        <h4 className="otp_heading">Verification 🔐</h4>
                        <span className="otp_subheading">
                            Enter the One-Time Password (OTP) sent to your registered mobile number or email address to complete the verification process.
                        </span>
                    </div>
                    ...
                    <button className="otp_button" onClick={() => { console.log('Button clicked'); verifyOtp(); }}>
                        <span>Verify OTP</span>
                    </button>

                    {/* Display error message if there is an error */}
                    {error && (
                        <div className="otp_error_message">
                            {error}
                        </div>
                    )}

                    {/* Trigger to resend OTP */}
                    <div className="otp_trigger">
                        <span><Link className="otp_trigger_btn" to="#">Resend OTP</Link></span>
                    </div>
                </div>
            </div>
        </div>
